...

{
  ...
  const [lo, hi] = SS4_DOMAIN;
  const out = SS4_RUNUP.bars.slice(0, SS4_LEAD).filter((b) => b.h > hi + 1e-9 || b.l < lo - 1e-9);
  if (out.length) throw new Error(`${out.length} run-up bars leave SS4's price range — they would draw past the panel`);
}

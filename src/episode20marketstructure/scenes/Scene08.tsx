...
const T = {
  ...
  lineOut: 226,
  swap: 234, // global 3742 — the hook's chart takes over
...
const LAST = { x: HOOK_GRID.x(HOOK_BARS.length - 1), y: HOOK_GRID.scale(HOOK_BARS[HOOK_BARS.length - 1].c) };

const bigWord = (text: string, size: number, color: string, at: number, f: number, out: number) => {
  const r = textReveal(f, at);
  return (
    <div style={{ fontSize: size, fontWeight: 800, color, opacity: r.opacity * out, transform: `translateY(${r.dy}px)`, lineHeight: 1.05 }}>{text}</div>
  );
};
...
export const Scene08 = () => {
  ...
  // ── the hook's chart replaces the descent ──
  const swap = f >= T.swap ? progress(f, T.swap, 20) : 0;
  ...
  return (
    <Stage>
      <Card>
        ...
        {convert < 0.999 && (
          <Layer opacity={chartOpacity} clip={{ x: BOX.x + BOX.w * convert, y: theme.stage.card.y, w: BOX.w * (1 - convert), h: theme.stage.card.h }}>
            <path d={pathOf(P.points)} fill="none" stroke={theme.color.ink} strokeWidth={theme.shape.line} strokeLinecap="round" strokeLinejoin="round" />
          </Layer>
        )}

        {swap < 0.999 && <CandleChart bars={DESC_BARS} box={BOX} reveal={convert} axis={false} opacity={chartOpacity * (1 - swap)} />}
        {swap > 0.001 && <CandleChart bars={HOOK_BARS} box={HOOK_BOX} ticks={HOOK_TICKS} tickLabels={false} opacity={chartOpacity * swap} />}
        ...
        {bigOut > 0.001 && (
          ...
          >
            ...
            {bigWord("Prinsip", BIG.word, theme.color.ink, T.words, f, bigOut)}
            {bigWord("Sederhana", BIG.word, theme.color.ink, T.words + 6, f, bigOut)}
          </div>
        )}

        {/* and then under it, where it can be read against the chart */}
        {sentenceOut > 0.001 && f >= T.line1 && (
          <div
            style={{
              position: "absolute",
              left: theme.canvas.width / 2,
          ...
          >
            <div style={{ color: theme.color.ink, opacity: l1.opacity, transform: `translateY(${l1.dy}px)` }}>Anggap tren berlanjut sampai</div>
            <div
              style={{
        ...
        />
        <Chip label="Mengenali perubahan" x={theme.canvas.width / 2} y={LINE_Y[1]} tone="indigo" at={T.know} />
      </Card>
    </Stage>
  );
};

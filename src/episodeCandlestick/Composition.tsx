...
import { Scene14A } from "./scenes/Scene14A";
...
const SceneFade: React.FC<{ durationInFrames: number; children: React.ReactNode }> = ({ durationInFrames, children }) => {
  ...
  const opacity = interpolate(f, [0, FADE, durationInFrames - FADE, durationInFrames], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
...
const INDEPENDENT_SCENES: { from: number; duration: number; Component: React.FC }[] = [
  ...
  { from: 9044, duration: 630, Component: Scene14A },
  { from: 9674, duration: 196, Component: Scene14B },
  { from: 9870, duration: 516, Component: Scene14C },
];

export const CandlestickComposition = () => {
  const f = useCurrentFrame();
  const watermarkFade = interpolate(f, [0, FADE, TOTAL_FRAMES - FADE, TOTAL_FRAMES], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: theme.colors.bg, fontFamily: theme.type.family }}>
      ...
      {/* Full-frame brand watermark — fades in at the start, out at the very end. */}

/**
 * SC19 — Recap: three conditions (from 9610, dur 512) — INDEPENDENT.
 *
 * The three shapes, side by side, drawn from the SAME structures the episode
 * taught them with — UPTREND from SC05/SC06, DOWNTREND from SC07, SIDEWAYS
 * from SC09. A recap that re-drew them freehand would be teaching a fourth
 * thing; these are literally the lines the viewer already read.
 *
 * The colour rule carries the recap on its own: indigo dots are peaks, cyan
 * dots are troughs, in all three cards.
 */
import { useCurrentFrame } from "remotion";
import { SafeArea } from "../components/SafeArea";
import { ChartCard } from "../components/ChartCard";
import { PriceLine } from "../components/PriceLine";
import { PivotMarker } from "../components/PivotMarker";
import { Chip } from "../components/Chip";
import { theme } from "../theme";
import { usePalette } from "../palette";
import { progress, textReveal } from "../helpers";
import {
  UPTREND,
  UP_PEAKS,
  UP_TROUGHS,
  DOWNTREND,
  DOWN_PEAKS,
  DOWN_TROUGHS,
  SIDEWAYS,
  SIDEWAYS_PEAKS,
  SIDEWAYS_TROUGHS,
  geom,
  type Structure,
} from "../data/structures";
import { CARD, CAPTION_Y } from "../layout";

// ═══ EDIT ═══════════════════════════════════════════════════════════════════
const T = {
  up: 82, // "higher high dan higher low"
  down: 182, // "lower high dan lower low"
  side: 263, // "keduanya tidak terbentuk"
  ring: 377, // "bukan berarti tidak terjadi apa-apa"
};
const GAP = 48;
const CARD_W = (CARD.w - GAP * 2) / 3;
const CARD_Y = 230;
const CARD_H = 520;
// ═══════════════════════════════════════════════════════════════════════════

const box = (i: number) => ({ x: CARD.x + i * (CARD_W + GAP), y: CARD_Y, w: CARD_W, h: CARD_H });
const plot = (i: number) => ({ x: box(i).x + 50, y: box(i).y + 150, w: CARD_W - 100, h: CARD_H - 250 });

const CARDS: {
  title: string;
  s: Structure;
  peaks: number[];
  troughs: number[];
  tags: [string, string];
  variant: "indigo" | "cyan";
  at: number;
}[] = [
  { title: "Uptrend", s: UPTREND, peaks: UP_PEAKS, troughs: UP_TROUGHS, tags: ["HH", "HL"], variant: "indigo", at: T.up },
  { title: "Downtrend", s: DOWNTREND, peaks: DOWN_PEAKS, troughs: DOWN_TROUGHS, tags: ["LH", "LL"], variant: "indigo", at: T.down },
  { title: "Sideways", s: SIDEWAYS, peaks: SIDEWAYS_PEAKS, troughs: SIDEWAYS_TROUGHS, tags: ["Seimbang", ""], variant: "cyan", at: T.side },
];

export const Scene19 = () => {
  const pal = usePalette();
  const f = useCurrentFrame();

  const ring = f >= T.ring ? progress(f, T.ring, 26) : 0;
  const sub = textReveal(f, T.ring + 20);

  return (
    <SafeArea>
      {CARDS.map((c, i) => {
        const g = geom(c.s, plot(i), { pad: 0.16 });
        const draw = f >= c.at ? progress(f, c.at + 8, 46) : 0;
        if (f < c.at) return null;
        return (
          <ChartCard key={c.title} box={box(i)} opacity={progress(f, c.at, 16)} radius={theme.radius.card}>
            <Chip label={c.title} x={box(i).x + CARD_W / 2} y={box(i).y + 76} variant={c.variant} startFrame={c.at} />
            <PriceLine g={g} draw={draw} color={pal.ink} width={3} />
            {draw > 0.9 &&
              c.peaks.map((pi) => {
                const p = g.pivot(pi);
                return <PivotMarker key={`p${pi}`} x={p.x} y={p.y} variant="indigo" startFrame={c.at + 52} />;
              })}
            {draw > 0.9 &&
              c.troughs.map((pi) => {
                const p = g.pivot(pi);
                return <PivotMarker key={`t${pi}`} x={p.x} y={p.y} variant="cyan" startFrame={c.at + 56} />;
              })}
            {c.tags[0] && (
              <Chip
                label={c.tags[0]}
                x={box(i).x + (c.tags[1] ? CARD_W / 2 - 60 : CARD_W / 2)}
                y={box(i).y + CARD_H - 58}
                variant="indigo"
                size={theme.type.axis.size}
                startFrame={c.at + 62}
              />
            )}
            {c.tags[1] && (
              <Chip label={c.tags[1]} x={box(i).x + CARD_W / 2 + 60} y={box(i).y + CARD_H - 58} variant="cyan" size={theme.type.axis.size} startFrame={c.at + 68} />
            )}
          </ChartCard>
        );
      })}

      {/* sideways is not nothing — it is undecided */}
      {ring > 0.001 && (
        <svg style={{ position: "absolute", left: 0, top: 0, overflow: "visible" }} width={theme.canvas.width} height={theme.canvas.height}>
          <rect
            x={box(2).x - 10}
            y={box(2).y - 10}
            width={CARD_W + 20}
            height={CARD_H + 20}
            rx={theme.radius.cardLg}
            fill="none"
            stroke={pal.cyan}
            strokeWidth={3}
            opacity={ring}
          />
        </svg>
      )}

      <div
        style={{
          position: "absolute",
          left: theme.canvas.width / 2,
          top: CAPTION_Y,
          transform: `translate(-50%, calc(-50% + ${sub.y}px))`,
          opacity: sub.opacity,
          fontFamily: theme.type.family,
          fontSize: theme.type.label.size,
          fontWeight: 600,
          color: pal.ink,
          whiteSpace: "nowrap",
        }}
      >
        Pasar belum menentukan arah.
      </div>
    </SafeArea>
  );
};

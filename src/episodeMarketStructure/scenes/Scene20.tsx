/**
 * SC20 — Foundation stack, the closer (from 10122, dur 412) — INDEPENDENT.
 *
 * The one conceptual visual in the episode. Everything before this earns its
 * place by being a real chart; a closing hierarchy has no chart to be, so it is
 * drawn as plain blocks: the base is what you read first, the tools rest on it.
 *
 * The base is as wide as all four tools together — deliberately, so the picture
 * says the tools cannot stand on anything narrower.
 */
import { useCurrentFrame } from "remotion";
import { SafeArea } from "../components/SafeArea";
import { theme } from "../theme";
import { usePalette } from "../palette";
import { textReveal, progress } from "../helpers";

// ═══ EDIT ═══════════════════════════════════════════════════════════════════
const T = {
  base: 0, // "membaca struktur harga"
  pulse: 167, // "arah dan bentuk pergerakannya"
  stack: 284, // "indikator atau alat bantu"
};
const BASE = { w: 1100, h: 116, cy: 700 };
const TOOLS = ["Indikator", "Volume", "Momentum", "Alat Bantu Lain"];
const TOOL = { w: 260, h: 100, gap: 20, cy: 552 };
const TAG_Y = 424;
const STEP = 16; // frames between one block landing and the next
// ═══════════════════════════════════════════════════════════════════════════

const CX = theme.canvas.width / 2;

export const Scene20 = () => {
  const pal = usePalette();
  const f = useCurrentFrame();

  const base = textReveal(f, T.base, 22, 40);
  // one breath on the base, no bounce
  const pulse = f >= T.pulse && f < T.pulse + 30 ? Math.sin(((f - T.pulse) / 30) * Math.PI) : 0;
  const tag = f >= T.stack + 80 ? progress(f, T.stack + 80, 20) : 0;

  return (
    <SafeArea>
      {/* the tools, each settling onto the base in turn */}
      {TOOLS.map((t, i) => {
        const r = textReveal(f, T.stack + i * STEP, 20, 26);
        const x = CX - (TOOL.w * 4 + TOOL.gap * 3) / 2 + i * (TOOL.w + TOOL.gap);
        return (
          <div
            key={t}
            style={{
              position: "absolute",
              left: x,
              top: TOOL.cy - TOOL.h / 2 + r.y,
              width: TOOL.w,
              height: TOOL.h,
              borderRadius: theme.radius.card,
              background: pal.cardBg,
              border: `${theme.stroke.hair}px solid ${pal.indigo}`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              fontFamily: theme.type.family,
              fontSize: theme.type.chip.size,
              fontWeight: theme.type.chip.weight,
              color: pal.indigo,
              opacity: r.opacity,
              boxSizing: "border-box",
              textAlign: "center",
              padding: "0 12px",
            }}
          >
            {t}
          </div>
        );
      })}

      {/* what everything else stands on */}
      <div
        style={{
          position: "absolute",
          left: CX - BASE.w / 2,
          top: BASE.cy - BASE.h / 2 + base.y,
          width: BASE.w,
          height: BASE.h,
          borderRadius: theme.radius.card,
          background: pal.indigo,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          fontFamily: theme.type.family,
          fontSize: theme.type.header.size,
          fontWeight: theme.type.header.weight,
          color: "#FFFFFF",
          opacity: base.opacity,
          transform: `scale(${1 + 0.012 * pulse})`,
          boxShadow: theme.shadow.rest,
        }}
      >
        Struktur Pergerakan Harga
      </div>

      {/* the tools' job, named once */}
      <div
        style={{
          position: "absolute",
          left: CX,
          top: TAG_Y,
          transform: "translate(-50%, -50%)",
          padding: "8px 20px",
          borderRadius: theme.radius.chip,
          background: pal.cyanSoft,
          border: `${theme.stroke.hair}px solid ${pal.cyan}`,
          color: pal.cyan,
          fontFamily: theme.type.family,
          fontSize: theme.type.chip.size,
          fontWeight: theme.type.chip.weight,
          opacity: tag,
          whiteSpace: "nowrap",
        }}
      >
        <span style={{ marginRight: 10 }}>✓</span>
        Konfirmasi
      </div>
    </SafeArea>
  );
};

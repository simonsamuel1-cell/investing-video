...
# The passages' own subtitles. The supplied SRTs are lower-case and
# unpunctuated; these keep every word as spoken and take the sentence case and
# punctuation of the existing Indonesian track.
PASSAGE_TEXT = {
    1: ["Jadi, kali ini", "kita nggak cuma melihat satu candle.", "Kita akan melihat bagaimana beberapa",
        "candle mulai membentuk cerita,", "bagaimana konteks membuat cerita itu lebih jelas,", "dan bagaimana",
        "membacanya di chart nyata."],
    2: ["Sekarang kita sudah melihat bahwa", "satu candle bisa memberi petunjuk.",
        "Tapi ketika beberapa candle mulai muncul bersama,", "ceritanya bisa menjadi jauh lebih jelas."],
    3: ["Sekarang kita sudah melihat beberapa pola yang berbeda.", "Tapi jangan berhenti", "di nama polanya.",
        "Yang lebih penting adalah melihat", "di mana pola itu muncul", "dan apa yang terjadi sebelumnya."],
    4: ["Jadi, kita sudah tahu cara membaca", "candle dan mengenali beberapa pola.",
        "Sekarang waktunya melihat semuanya bekerja bersama", "dalam satu chart nyata."],
}
...
def main():
    ...
    for p in PASSAGES:
        ...
        texts = PASSAGE_TEXT[p["n"]]
        assert len(srt) == len(texts), (p["n"], len(srt), len(texts))
        for block, text in zip(srt, texts):
            a, b = block.split("\n")[1].split(" --> ")
            ms = lambda s: (int(s[:2]) * 3600 + int(s[3:5]) * 60 + int(s[6:8])) * 1000 + int(s[9:12])
            out.append(dict(start=p["at"] + round(ms(a) * 0.03), end=p["at"] + round(ms(b) * 0.03), text=text))
    out.sort(key=lambda c: c["start"])
    for a, b in zip(out, out[1:]):
        assert a["end"] <= b["start"] + 0, ("overlap", a, b)
    sub = [
        "/**",
        " * subtitlesIndo.ts — Indonesian burned-in cues (frames @30fps) for TA03-CandlestickIntermediate.",
        " * GENERATED by scripts/ta02-indo-timeline.py from CSI_Sub_Indo_FIXED.srt",
        " * (scripts/ref/ta02-indo-cues.pre-extend.json) re-timed around Simon's four",
        " * passages, plus the passages' own cues. Do not hand-edit.",
        " */",
